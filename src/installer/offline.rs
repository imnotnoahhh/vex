...
pub(super) fn install_offline(
    ...
    let checksum = match expected_checksum {
        Some(checksum) => Some(checksum.trim().to_string()),
        None => archive_cache.get_archive_checksum(tool.name(), version, &archive_name)?,
    }
    .ok_or_else(|| {
        VexError::OfflineModeError(format!(
            "No checksum available for cached archive {}@{}. Reinstall once while online or use a lockfile generated by 'vex lock'.",
            tool.name(),
            version
        ))
    })?;
    progress.set_message("Verifying cached archive checksum");
    archive_cache.verify_checksum(&cached_archive, &checksum)?;
