...
3. Restores `~/.vex/npm` so shared npm globals survive cache hits
4. Re-activates tools after cache restore so `~/.vex/bin` and `~/.vex/npm/prefix/bin` are usable in later steps

Current scope:

- macOS runners only
- action implementation lives in this repository root (`action.yml`)
- cache targets are `~/.vex/cache`, `~/.vex/toolchains`, and `~/.vex/npm`
...
├── go/
│   └── env       # Go's managed GOENV file
├── npm/
│   ├── npmrc     # npm's managed user config
│   └── prefix/
│       └── bin/  # Shared npm global executables
├── python/
│   └── user/
│       └── bin/  # Python's managed pip --user executables
└── config.toml   # Configuration
```

Shell hooks generated by `vex env <shell>` also export `NPM_CONFIG_PREFIX=$HOME/.vex/npm/prefix` and `NPM_CONFIG_USERCONFIG=$HOME/.vex/npm/npmrc`, then prepend `~/.vex/npm/prefix/bin` before `~/.vex/bin`. This prefix is the shared npm globals pool for vex-managed Node versions, not a separate global prefix per Node version.
