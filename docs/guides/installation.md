...
3. Restores `~/.vex/npm` so managed npm globals survive cache hits
...
├── npm/
│   └── prefix/
│       └── bin/  # Managed npm global executables
└── config.toml   # Configuration
```

Shell hooks generated by `vex env <shell>` also export `NPM_CONFIG_PREFIX=$HOME/.vex/npm/prefix` and prepend `~/.vex/npm/prefix/bin` before `~/.vex/bin`.
